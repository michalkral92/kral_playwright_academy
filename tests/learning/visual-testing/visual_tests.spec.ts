...
test.describe("Visual Tests", () => {
  ...
  test("maxDiffPixelRation Test", async ({ page }) => {
    ...
    await expect(page).toHaveScreenshot("low_sensitivity_test.png", {
      ...
      // maxDiffPixels: 200 // ? Maximální počet rozdílný pixelů (tady 200)
    });
  });
  ...
  test("Hide Elements with CSS", async ({ page }) => {
    ...
    await expect(page).toHaveScreenshot("hide_elements_test.png", {
      ...
      stylePath: path.resolve(
        __dirname,
        "../../../src/pages/pmtool/asserts/visual_tests.css"
      ),
    });
  });

  test.only("Image Visual Test", async ({ page }) => {
  ...
  test.only("Module Visual Test", async ({ page }) => {
  ...
  test.only("Input Visual Test", async ({ page }) => {
